...
public class RCSCall {
    private static final String TAG = "RCSCall";

    public interface ICallUsersProvider {
        void onGotUserList(ArrayList<String> userIds);
    }

    private static int videoChatMemberCountMax = 9;
    private static int audioChatMemberCountMax = 20;

    private static String localizedAppName;

    private static GroupMembersProvider mGroupMembersProvider;

    private static RCSCallCustomerHandlerListener customerHandlerListener;
     ...
     */
    public static void startSingleCall(Context context, String targetId, RCSCallCommon.CallMediaType mediaType) {
        if (checkEnvironment(context, mediaType)) {
            ...
            intent.putExtra("callAction", RCSCallAction.ACTION_OUTGOING_CALL.getName());
            intent.setPackage(context.getPackageName());
            context.startActivity(intent);
        }
    }

    public static void starInComingCall(Context context, final RCSCallSession callSession, boolean startForCheckPermissions) {
        String action;
     ...
     */
    public static void startMultiCall(Context context, Conversation.ConversationType conversationType, String targetId, RCSCallCommon.CallMediaType mediaType, ArrayList<String> userIds) {
        if (checkEnvironment(context, mediaType)) {
            String action;
            if (mediaType.equals(RCSCallCommon.CallMediaType.AUDIO)) {
                action = RCSVoIPIntent.RONG_INTENT_ACTION_VOIP_MULTIAUDIO;
     ...
     */
    public static ICallUsersProvider startMultiCall(final Context context, final Conversation.ConversationType conversationType, final String targetId, final RCSCallCommon.CallMediaType mediaType) {
     ...
     * @param mediaType
     */
    public static void startMultiCall(final Context context, ArrayList<String> userIds, ArrayList<String> oberverIds, final RCSCallCommon.CallMediaType mediaType) {
        String action;
        if (mediaType.equals(RCSCallCommon.CallMediaType.AUDIO)) {
            action = RCSVoIPIntent.RONG_INTENT_ACTION_VOIP_MULTIAUDIO;
        } else {
     ...
     * @return
     */
    public static void startMultiCall(final Context context, ArrayList<String> userIds, final RCSCallCommon.CallMediaType mediaType) {
        if (checkEnvironment(context, mediaType)) {
            String action;
            if (mediaType.equals(RCSCallCommon.CallMediaType.AUDIO)) {
                action = RCSVoIPIntent.RONG_INTENT_ACTION_VOIP_MULTIAUDIO;
     ...
     * @return 是否允许启动通话界面
     */
    private static boolean checkEnvironment(Context context, RCSCallCommon.CallMediaType mediaType) {
     ...
     */
    public static void setCustomerHandlerListener(RCSCallCustomerHandlerListener callCustomerHandlerListener) {
     ...
     * 通话过程中用户自定义操作。
     */
    public static RCSCallCustomerHandlerListener getCustomerHandlerListener() {
    ...
    /*
     音频通话支持的最大通话人数
     */
    public static void setMaxMultiAudioCallUserNumber(int maxNumber) {
        audioChatMemberCountMax = maxNumber;
    }

    /*!
      视频通话支持的最大通话人数
    */
    public static void setMaxMultiVideoCallUserNumber(int maxNumber) {
        videoChatMemberCountMax = maxNumber;
    }

    public static int getMaxMultiAudioCallUserNumber() {
        return audioChatMemberCountMax;
    }

    public static int getMaxMultiVideoCallUserNumber() {
        return videoChatMemberCountMax;
    }

    /**
     来电显示的应用名字
     */
    public static void setAppLocalizedName(String localizedName){
        localizedAppName = localizedName;
    }

    public static String getAppLocalizedName() {
