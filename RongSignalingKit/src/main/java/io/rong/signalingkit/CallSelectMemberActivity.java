...
import io.rong.signalingkit.util.CallKitSearchBarListener;
import io.rong.signalingkit.util.CallKitSearchBarView;
...
public class CallSelectMemberActivity extends BaseNoActionBarActivity {
    ...
    ListView mList;
    RCSCallCommon.CallMediaType mMediaType;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int type = intent.getIntExtra("mediaType", RCSCallCommon.CallMediaType.VIDEO.getValue());
        mMediaType = RCSCallCommon.CallMediaType.valueOf(type);
        ...
        if (RCSCall.getGroupMemberProvider() != null) {
            List<String> groupMembers = RCSCall.getGroupMemberProvider().getMemberList(groupId, new RCSCall.OnGroupMembersResult() {
                @Override
    ...
    private AdapterView.OnItemClickListener adapterOnItemClickListener = new AdapterView.OnItemClickListener() {
        ...
        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
            ...
            if (!invitedMembers.contains(userId)) {
                ...
                if (selectedMember.size() >= inviteLimit) {
                    Toast.makeText(CallSelectMemberActivity.this, String.format(getString(R.string.rc_voip_invite_limit_hint), inviteLimit), Toast.LENGTH_SHORT).show();
                    if (selectedMember.contains(userId)) {
                        selectedMember.remove(userId);
                    }
                    v.setSelected(false);//1 false
                    callkit_conference_selected_number.setText(getString(R.string.callkit_selected_contacts_count, selectedMember.size()));
                    return;
                }
                int totalNumber = selectedMember.size() + invitedMembers.size();
                boolean videoObserverState = totalNumber >= (mMediaType.equals(RCSCallCommon.CallMediaType.AUDIO) ? RCSCall.getMaxMultiAudioCallUserNumber() : RCSCall.getMaxMultiVideoCallUserNumber());
                if (ctrlTag) {
                    if (videoObserverState) {
                        Toast.makeText(CallSelectMemberActivity.this, String.format(getString(mMediaType.equals(RCSCallCommon.CallMediaType.AUDIO) ? R.string.rc_voip_audio_numberofobservers : R.string.rc_voip_video_numberofobservers), totalNumber), Toast.LENGTH_SHORT).show();
                        return;
                    }
                    if (selectedMember.contains(userId)) {
                        selectedMember.remove(userId);
