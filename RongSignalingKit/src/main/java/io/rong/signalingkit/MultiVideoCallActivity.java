...
public class MultiVideoCallActivity extends BaseCallActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null && RCSCallManager.getInstance() == null) {
        ...
        boolean val = requestCallPermissions(RCSCallCommon.CallMediaType.VIDEO, REQUEST_CODE_ASK_MULTIPLE_PERMISSIONS);
    ...
    protected void onNewIntent(Intent intent) {
        ...
        boolean bool = requestCallPermissions(RCSCallCommon.CallMediaType.VIDEO, REQUEST_CODE_ASK_MULTIPLE_PERMISSIONS);
...
//                    RCSCallManager.getInstance().onPermissionGranted();
                } else {
                ...
                if (startForCheckPermissions) {
                    startForCheckPermissions = false;
//                    RCSCallManager.getInstance().onPermissionDenied();
                } else {
                    finish();
                }
            }
            ...
            if (resultCode == RESULT_OK) {
                ...
                List<RCSCallUserProfile> callUserProfiles = callSession.getParticipantProfileList();
                Iterator<String> iterator = invited.iterator();
                while (iterator.hasNext()) {
                    String id = iterator.next();
                    for (RCSCallUserProfile profile : callUserProfiles) {
                        if (profile.getUserId().equals(id)) {
                            iterator.remove();
                        }
                    }
                }
                RCSCallClient.getInstance().inviteRemoteUsers(invited);
            }
    ...
    public String onSaveFloatBoxState(Bundle bundle) {
        ...
        bundle.putString("callAction", RCSCallAction.ACTION_RESUME_CALL.getName());
        bundle.putInt("mediaType", RCSCallCommon.CallMediaType.VIDEO.getValue());
        return intentAction;
    }
    ...
    public void onRestoreFloatBox(Bundle bundle) {
        ...
        try {
            ...
            callSession = RCSCallClient.getInstance().getCallSession();
            if (bundle != null) {
                RCSCallAction callAction = RCSCallAction.valueOf(bundle.getString("callAction"));
                if (!callAction.equals(RCSCallAction.ACTION_RESUME_CALL)) {
                    return;
                }
     ...
     */
    @Override
    public void onCallOutgoing(final RCSCallSession callSession, SurfaceView localVideo) {
    ...
    public void onRemoteUserLeft(String userId, RCSCallCommon.CallDisconnectedReason reason) {
        RLog.i(TAG, "onRemoteUserLeft: userId " + userId + " reason " + reason);
        ...
        if (participantPortraitContainer != null) {
            ...
            RLog.i(TAG, "onRemoteUserLeft: participantView " + participantView);
        ...
        if (portraitContainer1 != null) {
            View childView = portraitContainer1.findViewWithTag(CallKitUtils.getStitchedContent(userId, VOIP_PARTICIPANT_PORTAIT_CONTAINER_TAG));
            RLog.i(TAG, "onRemoteUserLeft: childView " + childView);
            if (childView != null) {
                LinearLayout portraitContainer = (LinearLayout) childView.getParent();
                portraitContainer.removeView(childView);
            }
        }

        if (!TextUtils.isEmpty(localViewUserId) && localViewUserId.equals(userId)) {
        ...
        if (remoteViewContainer2 != null && !TextUtils.isEmpty(delUserid)) { //删除退出用户的头像框
            ...
            RLog.i(TAG, "onRemoteUserLeft: singleRemoteView " + singleRemoteView);
            if (singleRemoteView != null) {
        ...
        // 会话发起者离开
        RLog.i(TAG, "onRemoteUserLeft: userId " + userId + " getInviterUserId " + RCSCallClient.getInstance().getCallSession().getInviterUserId());
        if (TextUtils.equals(userId, RCSCallClient.getInstance().getCallSession().getInviterUserId()) && RCSCallClient.getInstance().getMyCallStatus() != RCSCallCommon.CallStatus.CONNECTED) {
            String findInvitor = RCSCallClient.getInstance().getCallSession().getParticipantUserIds().get(0);
            ...
            if (callSession != null) {
                UserInfo userInfo = RongContext.getInstance().getUserInfoFromCache(findInvitor);
                userNameView.setTag(CallKitUtils.getStitchedContent(findInvitor, VOIP_USERNAME_TAG));
                if (userInfo != null) {
                    userNameView.setText(userInfo.getName());
            ...
            onRemoteUserLeft(findInvitor, reason);
        }
    }
        ...
        if (callSession != null) {
            for (RCSCallUserProfile profile : callSession.getParticipantProfileList()) {
                if (profile.getUserId().equals(RongIMClient.getInstance().getCurrentUserId())) {
                    if (profile.getCallStatus().equals(RCSCallCommon.CallStatus.CONNECTED)) {
                        int callUserType = 1;
    ...
    public void onCallConnected(RCSCallSession callSession, SurfaceView localVideo) {
        ...
        if (localView == null) {
            ...
            observerLayout.setVisibility(callSession.getUserType() == RCSCallCommon.CallUserType.OBSERVER ? View.VISIBLE : View.GONE);
            localViewUserId = RongIMClient.getInstance().getCurrentUserId();
            localView.setTag(CallKitUtils.getStitchedContent(localViewUserId, REMOTE_FURFACEVIEW_TAG));
        }
    ...
    void updateRemoteVideoViews(RCSCallSession callSession) {
        String remoteUserID = "";
        ...
        for (RCSCallUserProfile profile : callSession.getParticipantProfileList()) {
            ...
            if (remoteUserID.equals(localViewUserId) || profile.getUserType() == RCSCallCommon.CallUserType.OBSERVER)
                continue;
...
//        ((RongRTCVideoView) video).setScalingType(RendererCommon.ScalingType.SCALE_ASPECT_FILL);
    ...
    public void onCallDisconnected(RCSCallSession callSession, RCSCallCommon.CallDisconnectedReason reason) {
        ...
        RongIM.getInstance().insertIncomingMessage(callSession.getConversationType(), callSession.getTargetId(), callSession.getCallerUserId(), receivedStatus, multiCallEndMessage, serverTime, null);
        cancelTime();
        stopRing();
        postRunnableDelay(new Runnable() {
            @Override
    ...
    protected void setupIntent() {
        ...
        if (callAction == null || callAction.equals(RCSCallAction.ACTION_RESUME_CALL)) {
            return;
        }
...
//                    iv_large_preview_Mask.setVisibility(View.VISIBLE);
                ...
                List<RCSCallUserProfile> list = callSession.getParticipantProfileList();
                List<String> incomingObserverUserList = callSession.getObserverUserList();
                for (RCSCallUserProfile profile : list) {
                ...
                for (int i = 0; i < invitedList.size(); i++) {
                    ...
                    Log.e(TAG, "onRemoteUserLeft: " + invitedList.get(i));
            ...
            RCSCallManager.getInstance().startCall(conversationType, targetId, userIds, RCSCallCommon.CallMediaType.VIDEO);
            FrameLayout bottomButtonLayout = (FrameLayout) inflater.inflate(R.layout.rc_voip_multi_video_calling_bottom_view, null);
            bottomButtonLayout.findViewById(R.id.rc_voip_call_mute).setVisibility(View.GONE);
            bottomButtonLayout.findViewById(R.id.rc_voip_disable_camera).setVisibility(View.GONE);
            bottomButtonLayout.findViewById(R.id.rc_voip_handfree).setVisibility(View.GONE);
    ...
    public void onReceiveBtnClick(View view) {
        if (acceptCallInvoked || callSession == null || isFinishing) {
            Log.e(TAG, "_接听多人视频出错 callSession=" + (callSession == null) + ",isFinishing=" + isFinishing);
            return;
        }
        ...
        RCSCallClient.getInstance().setMute(false);
        RCSCallClient.getInstance().setCameraEnabled(true);
    }
        ...
        if (callSession.getConversationType().equals(Conversation.ConversationType.DISCUSSION)) {
            RongIMClient.getInstance().getDiscussion(callSession.getTargetId(), new RongIMClient.ResultCallback<Discussion>() {
                ...
                public void onSuccess(Discussion discussion) {
                    ...
                    List<RCSCallUserProfile> list = RCSCallClient.getInstance().getCallSession().getParticipantProfileList();
                    for (RCSCallUserProfile profile : list) {
                        added.add(profile.getUserId());
                    }
                    ArrayList<String> allObserver = (ArrayList<String>) RCSCallClient.getInstance().getCallSession().getObserverUserList();
                    ...
                    intent.putExtra("mediaType", RCSCallCommon.CallMediaType.VIDEO.getValue());
    ...
    public void onMoreButtonClick(View view) {
        ...
        optionMenu.setHandUpvisibility(callSession.getUserType() == RCSCallCommon.CallUserType.OBSERVER);
    ...
    protected void onAddMember(List<String> newMemberIds) {
        ...
        RCSCallClient.getInstance().inviteRemoteUsers(added);
    }
    ...
    public void onMuteButtonClick(View view) {
        ...
        RCSCallClient.getInstance().setMute(isMuteMIC);
    }
    ...
    public void onDisableCameraBtnClick(View view) {
        ...
        RCSCallClient.getInstance().setCameraEnabled(view.isSelected());
        if (view.isSelected()) {
    ...
    public void onSwitchRemoteUsers(View view) {
        ...
        getObserverLayout();
        localViewContainer.addView(observerLayout);
        if (RCSCallClient.getInstance().getCallSession().getSelfUserId().equals(from) &&
                callSession.getUserType().getValue() == RCSCallCommon.CallUserType.OBSERVER.getValue()) {
...
//            FinLog.v("bugtags", "MultiVideoCallActivity 不在前台！");
        ...
        Log.i("bugtags", "Insert=" + headsetInfo.isInsert() + ",headsetInfo.getSignalingType=" + headsetInfo.getType().getValue());
        try {
