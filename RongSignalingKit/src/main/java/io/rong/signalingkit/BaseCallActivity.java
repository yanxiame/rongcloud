...
import io.rong.signalingkit.util.HeadsetPlugReceiver;
...
import static io.rong.signalingkit.CallFloatBoxView.showFB;
import static io.rong.signalingkit.util.CallKitUtils.isDebug;
import static io.rong.signalingkit.util.CallKitUtils.isDial;

/**
 * Created by weiqinxiao on 16/3/9.
 */
public class BaseCallActivity extends BaseNoActionBarActivity implements IRCSCallListener, PickupDetector.PickupDetectListener {
    ...
    public static boolean getHasShowed() {
        return hasShowed;
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        RCSCallListenerProxy.getInstance().registerCallListener(this);
    }
    ...
    @Override
    public void onCallOutgoing(RCSCallSession callProfile, SurfaceView localVideo) {
    ...
    public void onCallDisconnected(RCSCallSession callProfile, RCSCallCommon.CallDisconnectedReason reason) {
        if (RCSCall.getCustomerHandlerListener() != null) {
            RCSCall.getCustomerHandlerListener().onCallDisconnected(callProfile, reason);
        }
        ...
        switch (reason) {
            ...
            case OTHER_DEVICE_HAD_ACCEPTED:
                ...
                break;
            case REMOTE_BLOCKED:
                text = getString(R.string.rc_voip_reject_remote_blocked);
                break;
...
//                break;
    ...
    public void onRemoteUserInvited(String userId, RCSCallCommon.CallMediaType mediaType) {
        if (RCSCall.getCustomerHandlerListener() != null) {
            RCSCall.getCustomerHandlerListener().onRemoteUserInvited(userId, mediaType);
        }
    }
    ...
    public void onError(RCSCallCommon.CallErrorCode errorCode) {
        AudioPlayManager.getInstance().setInVoipMode(false);
        if (RCSCallCommon.CallErrorCode.ENGINE_NOT_FOUND.getValue() == errorCode.getValue() && isDebug(BaseCallActivity.this)) {
    ...
    @Override
    public void onCallConnected(RCSCallSession callProfile, SurfaceView localVideo) {
        if (RCSCall.getCustomerHandlerListener() != null) {
            RCSCall.getCustomerHandlerListener().onCallConnected(callProfile, localVideo);
        }
            ...
            if (checkDrawOverlaysPermission(true)) {
                if (action != null) {
                    ...
                    showOnGoingNotification(getString(R.string.rc_call_on_going),
                            mediaType == RCSCallCommon.CallMediaType.AUDIO.getValue()
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (RCSCall.getCustomerHandlerListener() != null) {
            List<String> selectedUserIds = RCSCall.getCustomerHandlerListener().handleActivityResult(requestCode, resultCode, data);
    ...
    protected void addMember(ArrayList<String> currentMemberIds) {
        ...
        if (RCSCall.getCustomerHandlerListener() != null) {
            RCSCall.getCustomerHandlerListener().addMember(this, currentMemberIds);
        }
    }
    ...
    @TargetApi(23)
    boolean requestCallPermissions(RCSCallCommon.CallMediaType type, int requestCode) {
        String[] permissions = null;
        Log.i(TAG, "BaseActivty requestCallPermissions requestCode=" + requestCode);
        if (type.equals(RCSCallCommon.CallMediaType.VIDEO) || type.equals(RCSCallCommon.CallMediaType.AUDIO)) {
