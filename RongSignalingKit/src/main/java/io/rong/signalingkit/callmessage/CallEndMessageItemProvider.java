...
import io.rong.signalingkit.RCSCallAction;
import io.rong.signalingkit.RCSCallClient;
import io.rong.signalingkit.RCSVoIPIntent;
...
public class CallEndMessageItemProvider extends IContainerItemProvider.MessageProvider<CallSTerminateMessage> {
    ...
    public void bindView(View v, int position, CallSTerminateMessage content, UIMessage data) {
        ...
        RCSCallCommon.CallMediaType mediaType = content.getMediaType();
...
//            case CONN_USER_BLOCKED:
        ...
        if (mediaType.equals(RCSCallCommon.CallMediaType.VIDEO)) {
            ...
            if (direction != null && direction.equals("MO")) {
                if (content.getReason().equals(RCSCallCommon.CallDisconnectedReason.HANGUP) ||
                        content.getReason().equals(RCSCallCommon.CallDisconnectedReason.REMOTE_HANGUP)) {
    ...
    public Spannable getContentSummary(Context context, CallSTerminateMessage data) {
        ...
        RCSCallCommon.CallMediaType mediaType = data.getMediaType();
    ...
    public void onItemClick(View view, int position, CallSTerminateMessage content, UIMessage message) {
        ...
        RCSCallSession profile = RCSCallClient.getInstance().getCallSession();
        if (profile != null && profile.getActiveTime() > 0) {
            Toast.makeText(view.getContext(),
                    profile.getMediaType() == RCSCallCommon.CallMediaType.AUDIO ?
