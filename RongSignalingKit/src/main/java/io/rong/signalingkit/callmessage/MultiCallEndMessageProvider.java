...
public class MultiCallEndMessageProvider extends IContainerItemProvider.MessageProvider<MultiCallEndMessage> {
    ...
    public void bindView(View v, int position, MultiCallEndMessage content, UIMessage message) {
        ...
        String msg = "";
        if (content.getReason() == RCSCallCommon.CallDisconnectedReason.OTHER_DEVICE_HAD_ACCEPTED) {
            msg = context.getResources().getString(R.string.rc_voip_call_other);
        } else if (content.getReason() == RCSCallCommon.CallDisconnectedReason.REMOTE_NO_RESPONSE) {
    ...
    public Spannable getContentSummary(Context context, MultiCallEndMessage message) {
        ...
        if (message.getReason() == RCSCallCommon.CallDisconnectedReason.NO_RESPONSE) {
