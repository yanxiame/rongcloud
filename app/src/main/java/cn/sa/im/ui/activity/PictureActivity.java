...
public class PictureActivity extends FragmentActivity implements View.OnLongClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mViewPager = (HackyViewPager) findViewById(io.rong.imkit.R.id.viewpager);
        ...
        if (!mMessage.getContent().isDestruct()) {
            ...
            //只显示1张照片
            ArrayList<PictureActivity.ImageInfo> lists = new ArrayList<>();
    ...
    public void onEventMainThread(Event.RemoteMessageRecallEvent event) {
        if (mCurrentMessageId == event.getMessageId()) {
            new AlertDialog.Builder(this, AlertDialog.THEME_DEVICE_DEFAULT_LIGHT)
                    ...
                    .setPositiveButton(getString(io.rong.imkit.R.string.rc_dialog_ok), new DialogInterface.OnClickListener() {
    ...
    public boolean onLongClick(View v) {
        ...
        if (imageInfo != null) {
            ...
            String[] items = new String[]{getString(io.rong.imkit.R.string.rc_save_picture)};
                ...
                public void onOptionsItemClicked(int which) {
                    if (which == 0) {
                        ...
                        if (file != null && file.exists()) {
                            ...
                            Toast.makeText(PictureActivity.this, PictureActivity.this.getString(io.rong.imkit.R.string.rc_save_picture_at), Toast.LENGTH_SHORT).show();
                        } else {
                            Toast.makeText(PictureActivity.this, getString(io.rong.imkit.R.string.rc_src_file_not_found), Toast.LENGTH_SHORT).show();
                        }
                    }
                }
            }).show();
    ...
    private class ImageAdapter extends PagerAdapter {
        private ArrayList<PictureActivity.ImageInfo> mImageList = new ArrayList<>();
        ...
        private View newView(final Context context, final PictureActivity.ImageInfo imageInfo) {
            View result = LayoutInflater.from(context).inflate(io.rong.imkit.R.layout.rc_fr_image, null);

            PictureActivity.ImageAdapter.ViewHolder holder = new PictureActivity.ImageAdapter.ViewHolder();
            holder.progressBar = result.findViewById(io.rong.imkit.R.id.rc_progress);
            holder.progressText = result.findViewById(io.rong.imkit.R.id.rc_txt);
            holder.photoView = result.findViewById(io.rong.imkit.R.id.rc_photoView);
            holder.mCountDownView = result.findViewById(io.rong.imkit.R.id.rc_count_down);
        ...
        public void addData(ArrayList<PictureActivity.ImageInfo> newImages, boolean direction) {
            if (newImages == null || newImages.size() == 0)
                return;
            if (mImageList.size() == 0) {
                mImageList.addAll(newImages);
            } else if (direction && !isFirstTime && !isDuplicate(newImages.get(0).getMessageId().getMessageId())) {
                ArrayList<PictureActivity.ImageInfo> temp = new ArrayList<>();
    ...
    private static class DestructListener implements RongIMClient.DestructCountDownTimerListener {
        private WeakReference<PictureActivity.ImageAdapter.ViewHolder> mHolder;
